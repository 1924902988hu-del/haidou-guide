...
def main():
    ...
    def aug_ref(aid, extra=None):
        ...
        out = {
            ...
            # 描述里偶含游戏内图标模板符(如 %i:scaleAP%),展示前剔除
            "desc": re.sub(r"%i:[A-Za-z0-9_]+%\s*", "", (hx or {}).get("description", "")),
        }
    ...
    for alias, c in sorted(champs.items()):
        ...
        if not hl or not hx:
            warn.append(f"{alias}: hero_list={bool(hl)} hexdata={bool(hx)}")
            continue
        ...
        detail = {
            ...
            "patch": {"game": game_patch, "ddragon": version, "hexdataDate": hx_build.get("reportDate")},
            "stats": stats,
    ...
    save_json(os.path.join(SITE_DATA, "index.json"), {
        "patch": {"game": game_patch, "ddragon": version,
